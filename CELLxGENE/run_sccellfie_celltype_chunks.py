...
output_directory = os.path.join(args.output_directory, basename)
...
celltype_col = 'cell_type' # column in CELLxGENE
...
for celltype, df in tqdm(adata.obs.groupby(celltype_col), desc='Running scCellFie in cells grouped by celltype'):
    ...
    # Keep only cells in "normal" tissues
    adata_tmp = adata_tmp[adata_tmp.obs.disease == 'normal']
    ...
    if adata_tmp.shape[0] > 2:       
        ...
        sc.pp.neighbors(adata_knn, n_neighbors=n_neighbors)
    ...
    try:
        ...
        print(f"Analysis of {celltype} failed. Continuing with next cell type.")
    # Delete variables to free memory
